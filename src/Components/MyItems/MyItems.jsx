...
const MyItems = () => {
  ...
  return (
    <div className="mb-10 font-mont ">
      <h1 className="text-center text-2xl lg:text-3xl font-bold mb-4 py-10 text-[#674636]">
        Bookmarked Items
      </h1>
      <div className="w-9/12 mx-auto lg:grid flex flex-col lg:grid-cols-3 gap-10">
        {bookmarkedItems.length > 0 ? (
          bookmarkedItems.map((item) => (
            <div key={item._id} className="card w-80  mx-auto bg-[#fff8e8] shadow-xl">
              <figure>
                <img
                  src={item.photoURL}
                  alt={item.name}
